...
layui.use(['layer', 'element', 'form', 'jquery'], function () {
  ...
  $('.layui-left-nav #nav').on('click', 'li', function (event) {
    ...
    //   $('.layui-left-nav').animate({
    //     width: '220px'
    //   }, 100);
    //   $('.layui-page-content,.dislpayArrow').animate({
    //     left: '220px'
    //   }, 100);
    //   $('.layui-left-nav i').removeClass('nav-iconfont');
    //   $('.layui-left-nav cite,.layui-left-nav .nav_right').show();
    //   $('.dislpayArrow a').removeClass("open");
    //   $('.indent-tab i').removeClass('layui-icon-spread-left');
    //   $('.indent-tab i').addClass('layui-icon-shrink-right');
    // }
  ...
  $('.layui-left-nav #nav').on('mouseenter', '.iconfont', function (event) {
    if ($('.layui-left-nav').css('width') == '60px') {
      var tips = $(this).next('cite').text();
      left_tips_index = layer.tips(tips, $(this));
    }
  })
  $('.layui-left-nav #nav').on('mouseout', '.iconfont', function (event) {
    layer.close(left_tips_index);
  })

  // 隐藏左侧
  $('.dislpayArrow a, .indent-tab i').click(function(event){
    if ($('.layui-left-nav').css('width') == '220px') {
      ...
      $('.layui-page-content,.dislpayArrow').animate({left: '60px'}, 100);
      $('.dislpayArrow a').addClass("open");
      $('.indent-tab i').removeClass('layui-icon-shrink-right');
      $('.indent-tab i').addClass('layui-icon-spread-left');
      $('.layui-left-nav #nav li .sub-menu li a').addClass('xs-left-nav');
    } else {
      $('.layui-left-nav').animate({width: '220px'}, 100);
      $('.layui-page-content,.dislpayArrow').animate({left: '220px'}, 100);
      $('.layui-left-nav i').removeClass('nav-iconfont');
      $('.layui-left-nav cite,.layui-left-nav .nav_right').show();
      $('.dislpayArrow a').removeClass("open");
      $('.indent-tab i').removeClass('layui-icon-spread-left');
  ...
  // 向右滚动标签
  $('.layui-tab .indent-tab.layui-icon-prev').click(function(event){
    rollPage('left');
  });

   // 向左滚动标签
   $('.layui-tab .indent-tab.layui-icon-next').click(function(event){
    rollPage();
  });
  ...
  $(window).resize(function() { 
    hgfull();
    if(!document.fullscreenElement){
      $("body").find('.layui-icon-screen-restore').removeClass('layui-icon-screen-restore').addClass('layui-icon-screen-full');
    }
  });
